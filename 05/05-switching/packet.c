...
//broadcast packet to all iface except the source one
void broadcast_packet(iface_info_t *iface, const char *packet, int len)
{
	iface_info_t *iface2 = NULL;
	list_for_each_entry(iface2, &instance->iface_list, list) {
		if (iface2->fd == iface->fd)
			continue;
		iface_send_packet(iface2, packet, len);
	}
}
