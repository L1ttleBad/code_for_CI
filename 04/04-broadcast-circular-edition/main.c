...
ustack_t *instance;
//from iface_list find corresponding iface
...
void iface_send_packet(iface_info_t *iface, const char *packet, int len)
{
	...
	struct ether_header *eh = (struct ether_header *)packet;  //assign packet address to eh
...
void broadcast_packet(iface_info_t *iface, const char *packet, int len)
{
	...
	iface_info_t * iface_ptr;
	static int i = 0;
	list_for_each_entry(iface_ptr, &instance->iface_list, list) {
		if ( iface_ptr != iface ){
			iface_send_packet( iface_ptr, packet , len);
			printf("%d: packet %s sended! dest iface: fd %d index %d name %s\n", i++, packet, iface_ptr->fd, iface_ptr->index, iface_ptr->name);
		}
	}
...
int open_device(const char *dname)
{
	...
	//store device index to ifr_ifindex
	struct ifreq ifr;
...
//put all available ifaces into instance->iface_list
