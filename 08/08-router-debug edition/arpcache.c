...
{
	//log(DEBUG,"start look up");
	pthread_mutex_lock(&arpcache.lock);
	//log(DEBUG,"locked");

/*	if(arpcache.entries[mac[0]%32].ip4 == ip4){
		int matched = 0;
		for(int i = 0; i < ETH_ALEN; i++){
			if(arpcache.entries[mac[0]%32].mac[i] == mac[i]){
				matched++;
			}
		}
		if(matched == ETH_ALEN){
			if(arpcache.entries[mac[0]%32].valid == 1){
				pthread_mutex_lock(&arpcache.lock);
				return 1;
			}
		}
	...
	if(arpcache.entries[ip4%32].ip4 == ip4){
		//log(DEBUG,"mac cache hit, ip is %x", ip4);
		for(int i = 0; i < ETH_ALEN; i++){
...
void arpcache_append_packet(iface_info_t *iface, u32 ip4, char *packet, int len)
{
	...
	struct arp_req * r;
	int matched = 0;
	log(DEBUG,"ip packet try pending %x", ip4);
	list_for_each_entry(r, &arpcache.req_list, list){
		log(DEBUG,"current req ip is %x", r->ip4);
		if(r->ip4 == ip4){
			log(DEBUG,"ip matched");
			if(r->iface == iface){
				log(DEBUG,"iface matched");
				matched = 1;
				break;
			}
		}
	...
	if(matched == 0){
	//if not, add new req
		log(DEBUG,"new ip packet pending");
		struct arp_req * new_req = (struct arp_req *)malloc(sizeof(struct arp_req));
		new_req->iface = iface;
		new_req->ip4 = ip4;
		log(DEBUG,"new req ip is %x", new_req->ip4);
	...
	log(DEBUG, "append and sent arp request, dest ip : %x ", ip4);
	...
	list_for_each_entry_safe(req, l, &arpcache.req_list, list){
		if(req->ip4 == ip4){
			list_for_each_entry(pkt, (struct list_head *)&req->cached_packets, list){
				...
				log(DEBUG,"pending packet has been sent, dst ip"IP_FMT,HOST_IP_FMT_STR(ip4));
				pthread_mutex_lock(&arpcache.lock);
			}
			delete_list(&req->cached_packets, struct cached_pkt, list);
			log(DEBUG,"req for ip "IP_FMT" is going to be deleted",HOST_IP_FMT_STR(ip4));
	...
	while (1) {
		...
		for(int i = 0; i < 32; i++){
			if((time(NULL) - arpcache.entries[i].added) > 15 && arpcache.entries[i].valid){
				 ...
				 log(DEBUG, "arp cache (ip) %hhu.%hhu.%hhu.%hhu is out of time and swept.\n", 
		...
		list_for_each_entry_safe(req, l, &arpcache.req_list, list){
			if(time(NULL) - req->sent > 1){
				if(req->retries < 5){				
					...
					log(DEBUG, "sent arp request, dest ip : %x , retries %d", req->ip4, ++req->retries);
				}
				else{
					list_for_each_entry(pkt, (struct list_head *)&req->cached_packets, list){
						pthread_mutex_unlock(&arpcache.lock);
					...
					delete_list(&req->cached_packets, struct cached_pkt, list);
					log(DEBUG,"req for ip "IP_FMT" is going to be deleted",HOST_IP_FMT_STR(req->ip4));
					list_delete_entry((struct list_head *)req);
					free(req);
				}
			}
