...
// lookup tcp sock in established_table with key (saddr, daddr, sport, dport) TODO
struct tcp_sock *tcp_sock_lookup_established(u32 saddr, u32 daddr, u16 sport, u16 dport)
{
	...
	//lookup the corresponsding hash entry list
	list_for_each_entry(sk, &tcp_established_sock_table[index], hash_list)
	{
		if(sk->sk_sip == saddr &&
		   sk->sk_sport == sport &&
		   sk->sk_dip == daddr &&
		   sk->sk_dport == dport)
		{	
			//log(DEBUG,"find establisted, daddr:"IP_FMT,HOST_IP_FMT_STR(daddr));
		   	return sk;
		}
	}
...
//
// In accordance with BSD socket, saddr is in the argument list, but never used. TODO
struct tcp_sock *tcp_sock_lookup_listen(u32 saddr, u16 sport)
{
	int index = tcp_hash_function(0, 0, sport, 0);  //should be -1, but 0 is ok, too.
	struct tcp_sock * sk;
	
	//lookup the corresponsding hash entry list
	list_for_each_entry(sk, &tcp_listen_sock_table[index], hash_list)
	{
		if(sk->sk_sport == sport)
		{	
			//log(DEBUG,"find listen, sport: %d", sport);
		   	return sk;
		}
	}
...
// connect to the remote tcp sock specified by skaddr TODO
//
// XXX: skaddr here contains network-order variables
...
int tcp_sock_connect(struct tcp_sock *tsk, struct sock_addr *skaddr)
{
	...
	tsk->snd_nxt = tsk->iss;
	//log(DEBUG,"connect seq : %d",tsk->snd_nxt);
	tsk->rcv_nxt = 0;
	tsk->snd_una = 0;
	

	//2
	tcp_sock_set_sport(tsk, tsk->sk_sport);

	//3
	//char * packet = (char *)malloc(ETHER_HDR_SIZE + IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE);
	//tcp_flag_send_packet(tsk, TCP_SYN, packet, ETHER_HDR_SIZE + IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE);
...
// set backlog (the maximum number of pending connection requst), switch the TODO
...
// if accept_queue is not emtpy, pop the first tcp sock and accept it, TODO
...
struct tcp_sock *tcp_sock_accept(struct tcp_sock *tsk)
{
	...
	//tcp_set_state(new_tsk, TCP_ESTABLISHED);
	tsk->accept_backlog++;
	return new_tsk;
}
